...
//  CreateInstance
//
//  Static class method to create the WMFCapture object.
//-------------------------------------------------------------------

HRESULT WMFCapture::CreateInstance(
	HWND     hwnd,       // Handle to the window to receive events
	WMFCapture **ppCapture // Receives a pointer to the WMFCapture object.
)
{
	if (ppCapture == NULL)
	{
	...
	WMFCapture *pCapture = new (std::nothrow) WMFCapture(hwnd);
	...
	// The WMFCapture constructor sets the ref count to 1.
	*ppCapture = pCapture;
...
WMFCapture::WMFCapture(HWND hwnd) :
	m_pReader(NULL),
	m_pWriter(NULL),
	m_hwndEvent(hwnd),
	m_nRefCount(1),
...
HRESULT WMFCapture::QueryInterface(REFIID riid, void** ppv)
{
	static const QITAB qit[] =
	{
		QITABENT(WMFCapture, IMFSourceReaderCallback),
		{ 0 },
	};
...
HRESULT WMFCapture::OnReadSample(
	HRESULT hrStatus,
	DWORD /*dwStreamIndex*/,
	DWORD /*dwStreamFlags*/,
...
HRESULT WMFCapture::OpenMediaSource(IMFMediaSource *pSource)
{
	HRESULT hr = S_OK;
...
HRESULT WMFCapture::StartCapture(
...
HRESULT WMFCapture::EndCaptureSession()
{
	EnterCriticalSection(&m_critsec);
...
HRESULT WMFCapture::CheckDeviceLost(DEV_BROADCAST_HDR *pHdr, BOOL *pbDeviceLost)
{
...
HRESULT WMFCapture::ConfigureCapture(const EncodingParameters& param)
{
	HRESULT hr = S_OK;
	DWORD sink_stream = 0;
...
HRESULT WMFCapture::EndCaptureInternal()
{
	HRESULT hr = S_OK;
